...
class SemanticScholarSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        data['references'] = response.xpath(self.CITATIONS).getall()
        data['authors'] = response.xpath(self.AUTHORS).getall()
        data['abstract'] = response.xpath(self.ABSTRACT).get()

        self.data_dict.append(info)
        with open('./elasticsearch/data.json', 'w+') as f:
            f.write(dumps(self.data_dict))
        counter = 0
        for citation in data['references']:
